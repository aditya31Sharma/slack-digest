...
function loadStores() {
  ...
  for (const [k, v] of Object.entries(process.env)) {
    if (k.endsWith('_SHOPIFY_ACCESS_TOKEN') || k.endsWith('_SHOPIFY_DOMAIN')) src[k] = v;
  }
  if (!Object.keys(src).some(k => k.endsWith('_SHOPIFY_ACCESS_TOKEN')) && fs.existsSync(SHOPIFY_ENV_PATH)) {
    for (const line of fs.readFileSync(SHOPIFY_ENV_PATH, 'utf8').split('\n')) {
      const t = line.trim();
  ...
  for (const k of Object.keys(src)) {
    if (!k.endsWith('_SHOPIFY_ACCESS_TOKEN')) continue;
    const name = k.replace('_SHOPIFY_ACCESS_TOKEN', '');
    const dom = src[`${name}_SHOPIFY_DOMAIN`];
    if (dom) stores[name] = { token: src[k], domain: dom };
  }
  return stores;
}
...
async function storeMetrics({ token, domain }, range) {
...
async function fetchSessions({ token, domain }, range) {
  try {
...
async function fetchDigest({ brandKeys = [], range, withSessions = false }) {
  const stores = loadStores();
  ...
  return { range, brands: ok, noAccess, errored, unknown: [], totals, dailyAll, ccy };
}
...
Each digest shows *Revenue · Orders · AOV · Highest order · Sessions · Conversion*.
...
function digestBlocks(report) {
  ...
  if (brands.length > 1) {
    blocks.push({ type: 'divider' });
    const lines = brands.map((r, i) => `${i < 3 ? ['🥇','🥈','🥉'][i] + ' ' : ''}*${r.brand}* — ${fmtMoney(r.revenue, r.currency)} · ${fmtNum(r.orders)} ord · ${fmtMoney(r.aov, r.currency)} AOV`);
...
// ── Deep digest: standalone HTML report with charts ─────────────────────────
function buildDeepDigestHtml(report) {
  ...
  const kpis = [
    ['Revenue', money(totals.revenue)], ['Orders', fmtNum(totals.orders)], ['AOV', money(totals.aov)],
    ['Highest order', money(totals.maxOrder)], ['Sessions', totals.sessionsKnown ? fmtNum(totals.sessions) : 'n/a'],
    ['Brands', String(brands.length)],
  ];
  const rows = brands.map(b => `<tr><td>${b.brand}</td><td>${money(b.revenue)}</td><td>${fmtNum(b.orders)}</td><td>${money(b.aov)}</td><td>${money(b.maxOrder)}</td><td>${b.sessions != null ? fmtNum(b.sessions) : '—'}</td></tr>`).join('');
  return `<!doctype html><html lang=en><head><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1">
<title>Deep digest · ${scope} · ${range.label}</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4"></script>
<style>
:root{--bg:#0b0b0d;--panel:#15151a;--line:#26262e;--fg:#f4f4f6;--mut:#9a9aa3;--acc:#ff3b3b}
*{box-sizing:border-box}body{margin:0;background:radial-gradient(1200px 600px at 75% -10%,#1b1b24,#0b0b0d 60%);color:var(--fg);font:15px/1.5 ui-sans-serif,-apple-system,Segoe UI,Roboto,sans-serif}
.wrap{max-width:1080px;margin:0 auto;padding:34px 22px 70px}
h1{font:800 26px/1.1 'Helvetica Neue',Arial;margin:0 0 4px;text-transform:uppercase;letter-spacing:.01em}
...
.kpi .l{color:var(--mut);font-size:12px;text-transform:uppercase;letter-spacing:.08em}
.kpi .v{font:800 26px/1.2 'Helvetica Neue',Arial;margin-top:6px}
.card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:18px 20px;margin-bottom:20px}
.card h2{font-size:13px;text-transform:uppercase;letter-spacing:.1em;color:var(--mut);margin:0 0 14px}
table{width:100%;border-collapse:collapse;font-size:14px}
td,th{padding:10px 12px;text-align:left;border-bottom:1px solid #1f1f27}
...
<div class=card><h2>Revenue by brand</h2><canvas id=byBrand height=120></canvas></div>
<div class=card><h2>Daily revenue</h2><canvas id=daily height=110></canvas></div>
<div class=card><h2>Per-brand breakdown</h2><table><tr><th>Brand</th><th>Revenue</th><th>Orders</th><th>AOV</th><th>Highest</th><th>Sessions</th></tr>${rows}</table>
${totals.sessionsKnown ? '' : '<div class=foot>Sessions show n/a where the store token lacks the read_analytics scope.</div>'}</div>
<div class=foot>Brand Sales bot · deepdigest</div>
<script>
const ax={scales:{x:{ticks:{color:'#9a9aa3'},grid:{color:'#1f1f27'}},y:{ticks:{color:'#9a9aa3'},grid:{color:'#1f1f27'}}},plugins:{legend:{display:false}}};
new Chart(byBrand,{type:'bar',data:{labels:${JSON.stringify(brandNames)},datasets:[{data:${JSON.stringify(brandRev)},backgroundColor:'#ff3b3b',borderRadius:6}]},options:ax});
new Chart(daily,{type:'line',data:{labels:${JSON.stringify(days)},datasets:[{data:${JSON.stringify(dailyRev)},borderColor:'#ff8a4d',backgroundColor:'rgba(255,138,77,.15)',fill:true,tension:.3,pointRadius:2}]},options:ax});
